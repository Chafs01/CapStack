  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}
      ...
      <div className="glass" style={{width:'100%',maxWidth:420,padding:'32px 28px',position:'relative'}}>
        ...
        {mode!=='reset'&&<button onClick={google} disabled={busy} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:10,padding:'11px 16px',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:8,fontSize:'var(--fs-5)',fontWeight:600,color:'var(--text)',cursor:'pointer',fontFamily:"'Sora',sans-serif",marginBottom:EMAIL_AUTH?18:8}}>
        ...
        {EMAIL_AUTH&&<>
          ...
          {mode==='login'&&<button onClick={()=>switchMode('reset')} style={{background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontSize:'var(--fs-3)',padding:0,marginTop:-6,marginBottom:8,fontFamily:"'Sora',sans-serif"}}>Forgot password?</button>}
        </>}
        ...
        {EMAIL_AUTH&&<div style={{textAlign:'center',fontSize:'var(--fs-4)',color:'var(--muted)'}}>
          {mode==='login'&&<>No account? <button onClick={()=>switchMode('signup')} style={{background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontWeight:600,fontSize:'var(--fs-4)',fontFamily:"'Sora',sans-serif"}}>Sign Up</button></>}
          {mode==='signup'&&<>Have an account? <button onClick={()=>switchMode('login')} style={{background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontWeight:600,fontSize:'var(--fs-4)',fontFamily:"'Sora',sans-serif"}}>Sign In</button></>}
          {mode==='reset'&&<button onClick={()=>switchMode('login')} style={{background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontWeight:600,fontSize:'var(--fs-4)',fontFamily:"'Sora',sans-serif"}}>← Back to sign in</button>}
        </div>}
      </div>
    </div>
  );
...
function Toast({msg}){
  if(!msg)return null;
  return <div className="fu" style={{position:'fixed',bottom:26,left:'50%',transform:'translateX(-50%)',background:'var(--ink)',color:'#fff',padding:'11px 22px',borderRadius:10,fontSize:'var(--fs-4)',fontWeight:600,zIndex:600,boxShadow:'var(--shadow-lg)',whiteSpace:'nowrap'}}>{msg}</div>;
}
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}
      ...
      <div className="glass" style={{width:'100%',maxWidth:420,padding:'30px 28px',position:'relative'}}>
        ...
        {!user&&<button onClick={onSignIn} style={{display:'block',margin:'14px auto 0',background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontWeight:600,fontSize:'var(--fs-4)',fontFamily:"'Sora',sans-serif"}}>Sign in to save to the cloud</button>}
      </div>
    </div>
  );
}
