    ...
    <div className="fu">
      <div className="dark-surface grid-bg" style={{position:'relative',overflow:'hidden'}}>
        ...
        <div className="g2" style={{position:'relative',maxWidth:1140,margin:'0 auto',padding:'72px 24px 64px',display:'grid',gridTemplateColumns:'1.05fr .95fr',gap:56,alignItems:'center'}}>
          <div>
            <div className="mono" style={{fontSize:'var(--fs-2)',fontWeight:600,letterSpacing:'3px',color:'#7d93ff',marginBottom:20,display:'flex',alignItems:'center',gap:10}}>
            ...
            <p style={{fontSize:'var(--fs-6)',color:'#aab3c9',lineHeight:1.6,marginBottom:30,maxWidth:520}}>Enter your assumptions. Get a full ten-year discounted cash flow, levered returns, an equity waterfall, sensitivity analysis, and a lender-ready Excel model in under two minutes.</p>
            <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:24}}>
              <button className="btn-p" style={{padding:'13px 30px',fontSize:'var(--fs-5)'}} onClick={onStart}>Start an analysis</button>
              <button style={{padding:'12px 26px',fontSize:'var(--fs-5)',background:'rgba(255,255,255,.06)',color:'#fff',border:'1px solid rgba(255,255,255,.18)',borderRadius:8,fontWeight:600,cursor:'pointer',fontFamily:"'Sora',sans-serif"}} onClick={onSample}>Download sample model</button>
            </div>
            <div className="mono" style={{fontSize:'var(--fs-2)',color:'#6b7593',letterSpacing:'.5px'}}>MULTIFAMILY / COMMERCIAL / MIXED-USE / DEVELOPMENT / LIHTC</div>
          </div>
          <div style={{borderRadius:14,overflow:'hidden',border:'1px solid rgba(255,255,255,.12)',background:'rgba(18,26,46,.7)',backdropFilter:'blur(8px)',boxShadow:'0 24px 60px rgba(0,0,0,.45)'}}>
            <div style={{borderBottom:'1px solid rgba(255,255,255,.08)',padding:'13px 18px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:'var(--fs-4)',fontWeight:700,color:'#fff'}}>40-Unit Multifamily Acquisition</span>
              <span className="mono" style={{fontSize:'var(--fs-2)',color:'#6b7593'}}>7-YR HOLD</span>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',borderBottom:'1px solid rgba(255,255,255,.08)'}}>
              {[{l:'LEVERED IRR',v:f.pct(r.ret.irr,1)},{l:'EQUITY MULT.',v:f.x(r.ret.em)},{l:'YR-1 DSCR',v:r.sum.dscr.toFixed(2)+'x'}].map((k,i)=>(
                <div key={k.l} style={{padding:'16px 12px',textAlign:'center',borderRight:i<2?'1px solid rgba(255,255,255,.08)':'none'}}>
                  <div className="mono" style={{fontSize:'var(--fs-8)',fontWeight:700,color:'#7d93ff'}}>{k.v}</div>
                  <div className="mono" style={{fontSize:'var(--fs-1)',color:'#6b7593',marginTop:4,letterSpacing:'.5px'}}>{k.l}</div>
                </div>
              ))}
            </div>
            <div style={{padding:'17px 18px'}}>
              <div className="mono" style={{fontSize:'var(--fs-1)',fontWeight:600,color:'#6b7593',marginBottom:12,letterSpacing:'1px'}}>NET OPERATING INCOME</div>
              <div style={{display:'flex',alignItems:'flex-end',gap:8,height:84}}>
                {yrs.map(x=>(
                  <div key={x.yr} style={{flex:1,textAlign:'center'}}>
                    ...
                    <div className="mono" style={{fontSize:'var(--fs-1)',color:'#6b7593',marginTop:6}}>Y{x.yr}</div>
                  </div>
                ))}
              </div>
              <div style={{display:'flex',justifyContent:'space-between',marginTop:14,paddingTop:13,borderTop:'1px solid rgba(255,255,255,.08)',fontSize:'var(--fs-3)'}}>
      ...
      <div style={{maxWidth:1140,margin:'0 auto',padding:'64px 24px 8px'}}>
        <div className="mono" style={{fontSize:'var(--fs-2)',fontWeight:600,letterSpacing:'3px',color:'var(--accent)',marginBottom:14}}>HOW IT WORKS</div>
        <h2 style={{fontSize:'clamp(24px,3.4vw,32px)',fontWeight:600,marginBottom:8}}>From assumptions to pro forma in three steps.</h2>
        <p style={{color:'var(--muted)',fontSize:'var(--fs-5)',maxWidth:560,lineHeight:1.6}}>No spreadsheet wrangling, no template licenses. The model is built the moment you finish typing.</p>
        <div className="g3" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginTop:28}}>
          {[
            ...
            <div key={s.n} className="glass" style={{padding:'24px 22px'}}>
              <div className="mono" style={{fontSize:'var(--fs-3)',fontWeight:700,color:'var(--accent)',marginBottom:12}}>{s.n}</div>
              <div style={{fontSize:'var(--fs-5)',fontWeight:700,marginBottom:8,fontFamily:"'Space Grotesk',sans-serif"}}>{s.t}</div>
              <div style={{fontSize:'var(--fs-4)',color:'var(--muted)',lineHeight:1.6}}>{s.d}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* FEATURES */}
      <div style={{maxWidth:1140,margin:'0 auto',padding:'64px 24px 8px'}}>
        <div className="mono" style={{fontSize:'var(--fs-2)',fontWeight:600,letterSpacing:'3px',color:'var(--accent)',marginBottom:14}}>EVERYTHING IN THE MODEL</div>
        ...
        <div className="g3" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginTop:28}}>
          {[
            ...
            <div key={x.t} className="glass" style={{padding:'22px 20px'}}>
              ...
              <div style={{fontSize:'var(--fs-5)',fontWeight:700,marginBottom:7,fontFamily:"'Space Grotesk',sans-serif"}}>{x.t}</div>
              <div style={{fontSize:'var(--fs-4)',color:'var(--muted)',lineHeight:1.55}}>{x.d}</div>
            </div>
          ))}
        </div>
        <div className="g2" style={{marginTop:24,display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'8px 24px'}}>
          {[
            ...
            <div key={x} style={{display:'flex',gap:9,alignItems:'baseline',fontSize:'var(--fs-4)',color:'var(--muted)'}}>
      ...
      <div className="dark-surface grid-bg" style={{marginTop:64,position:'relative',overflow:'hidden'}}>
        ...
        <div className="g2" style={{position:'relative',maxWidth:1140,margin:'0 auto',padding:'64px 24px',display:'grid',gridTemplateColumns:'1fr .9fr',gap:56,alignItems:'center'}}>
          <div>
            <div className="mono" style={{fontSize:'var(--fs-2)',fontWeight:600,letterSpacing:'3px',color:'#7d93ff',marginBottom:14}}>EXCEL EXPORT</div>
            ...
            <p style={{fontSize:'var(--fs-5)',color:'#aab3c9',lineHeight:1.65,marginBottom:18}}>Most tools hand you a PDF or a wall of hardcoded numbers. SmartCapStack writes a real workbook: every output is a live Excel formula wired to the input cells, including the IRR sensitivity grid.</p>
            <p style={{fontSize:'var(--fs-5)',color:'#8a93a6',lineHeight:1.6,marginBottom:26}}><span style={{color:'#7db4e8',fontWeight:700}}>Blue cells</span> are inputs. Black cells are formulas. Change an assumption in Excel and the whole model — pro forma, returns, sensitivity — recalculates. Send it straight to a lender or LP.</p>
            <button style={{padding:'12px 26px',fontSize:'var(--fs-5)',background:'rgba(255,255,255,.06)',color:'#fff',border:'1px solid rgba(255,255,255,.18)',borderRadius:8,fontWeight:600,cursor:'pointer',fontFamily:"'Sora',sans-serif"}} onClick={onSample}>Download sample model</button>
          </div>
          <div style={{borderRadius:12,overflow:'hidden',border:'1px solid rgba(255,255,255,.12)',boxShadow:'0 24px 60px rgba(0,0,0,.45)'}}>
            <div style={{background:'#1f3864',padding:'10px 16px',display:'flex',alignItems:'center',gap:9}}>
              ...
              <span className="mono" style={{fontSize:'var(--fs-2)',color:'#fff',fontWeight:600}}>SmartCapStack_Model.xlsx</span>
            </div>
            <div className="mono" style={{padding:'8px 14px',borderBottom:'1px solid var(--border2)',fontSize:'var(--fs-2)',color:'#5a6478',display:'flex',gap:10,background:'#fff'}}>
            ...
            {[
              ...
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'9px 16px',borderBottom:'1px solid var(--border)',fontSize:'var(--fs-3)',background:i%2?'var(--surface2)':'#fff'}}>
      ...
      {/* PIPELINE */}
      <div style={{maxWidth:1140,margin:'0 auto',padding:'64px 24px 8px'}}>
        <div className="mono" style={{fontSize:'var(--fs-2)',fontWeight:600,letterSpacing:'3px',color:'var(--accent)',marginBottom:14}}>YOUR PIPELINE</div>
      ...
      <div style={{maxWidth:1140,margin:'0 auto',padding:'56px 24px 72px'}}>
        <div className="dark-surface" style={{borderRadius:16,padding:'56px 32px',textAlign:'center',position:'relative',overflow:'hidden'}}>
          ...
          <p style={{color:'#aab3c9',fontSize:'var(--fs-5)',marginBottom:26,position:'relative'}}>Free while in early access. Runs entirely in your browser — nothing to install.</p>
          <button className="btn-p" style={{padding:'13px 34px',fontSize:'var(--fs-5)',position:'relative'}} onClick={onStart}>Start an analysis</button>
        </div>
      </div>
    </div>
  );
